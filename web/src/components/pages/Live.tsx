...
const LiveStreamCard: React.FC<{ stream: LiveStream }> = ({ stream }) => {
    ...
    return (
        <a href={stream.stream_url} target="_blank" rel="noopener noreferrer" className="bg-[#1E1E1E] rounded-lg overflow-hidden shadow-sm transition-all duration-300 border border-white/10 hover:border-[#387B66]/80 hover:shadow-lg hover:shadow-[#387B66]/10 hover:-translate-y-1 block group">
            <div className="p-5">
                <div className="flex items-start space-x-4">
                    <img src={stream.avatar_url} alt={stream.streamer_name} className={`w-16 h-16 rounded-full border-2 border-white/20 group-hover:border-[#387B66] transition-colors`} />
