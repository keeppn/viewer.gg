          ...
          <tbody>
            {filteredApplications.map(app => (
              <tr key={app.id} className="border-b border-white/10 hover:bg-black/20">
                ...
                <td className="p-4 text-gray-300">{app.streamer.follower_count.toLocaleString()}</td>
                <td className="p-4 text-gray-300">{app.tournament?.title || 'N/A'}</td>
                <td className="p-4 text-gray-400">{new Date(app.submission_date).toLocaleDateString()}</td>
                <td className="p-4">
