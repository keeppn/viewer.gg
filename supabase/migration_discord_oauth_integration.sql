...
CREATE TABLE IF NOT EXISTS discord_role_history (
  ...
  organizer_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,  tournament_id UUID REFERENCES tournaments(id) ON DELETE SET NULL,
...
-- Add Discord-related columns to tournament_applications table
...
-- Add Discord ID to streamer profiles
ALTER TABLE streamer_profiles
ADD COLUMN IF NOT EXISTS discord_id TEXT,
ADD COLUMN IF NOT EXISTS discord_username TEXT;

-- Create indices for better performance
CREATE INDEX idx_discord_integrations_organizer ON discord_integrations(organizer_id);
CREATE INDEX idx_discord_role_history_organizer ON discord_role_history(organizer_id);
CREATE INDEX idx_discord_role_history_timestamp ON discord_role_history(timestamp DESC);

-- Enable Row Level Security
ALTER TABLE discord_integrations ENABLE ROW LEVEL SECURITY;ALTER TABLE discord_role_history ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "Organizers can view their role assignment history"
  ON discord_role_history  FOR SELECT
  USING (auth.uid() = organizer_id);
